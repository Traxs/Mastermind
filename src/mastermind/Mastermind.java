...
/**
 * This class is basically the heart out of the whole project. inside of it is
 * the structure and the control of every other object/class. The Class
 * Mastermind implements Serializable for saving the whole Object.
 * <p>
 * <p>
 * 
 * @author Birk Kauer
...
public class Mastermind implements java.io.Serializable
{
    ...
    /**
     * The KI. KI is transient for not beeing put into file via Serializable
     * 
     * @see mastermind.KI
     */
     ...
     * <p>
     * Parse the Gui Window into the Mastermind Object and calling the big
     * overloaded Constructor with default values.
     * <p>
     * 
     * @param mastermind_View
     *            the mastermind_ view
     * @see view.Mastermind_View
     */
     ...
     * <p>
     * then it will call
     * {@link file.Mastermind_File#saveMastermind(Mastermind, File)} with Values
     ...
     * Load mastermind.
     * <p>
     * it will call {@link file.Mastermind_File#loadMastermind(File)} and load
     * every Instances of the loaded mastermind object into the current object
     * <p>
     * It will also create a new Instance of KI so it can calculate from
     * scratch.
     * <p>
     * 
     * @param file
     ...
     */
    public void loadMastermind(File file) throws ClassNotFoundException,
            IOException
    {
     ...
     * Gets the hint.
     * <p>
     * getHint will start the calculating functions in KI with
     * {@link mastermind.KI#getHint()} if the KI isn't already calculating which
     * is checked via {@link mastermind.KI#isKICalculating()}
     * <p>
     * 
     * @see mastermind.Mastermind#state
     ...
     * Finish hint.
     * <p>
     * finishHint is called from KI when the calculation of the next hint is
     * done.
     * <p>
     * it will add a hint with {@link view.Mastermind_View#addHint(int[])} and
     * the parameter is the calculated Hint code.
     * <p>
     * at the End the function will set the current state into an updated state(
     * {@link mastermind.Mastermind#setState(State)}.
     * <p>
     * 
     * @param code
     *            the code
     ...
     * <p>
     * isPossible will ask the KI (as long as the KI is not calculating) if the
     * current selected Code is possible to be selected.
     * <p>
     * 
     * @param code
     *            the code
     ...
     * Finish check.
     * <p>
     * Last check if the Code is possible with the current calculations to be
     * set on the field.
     * <p>
     * If it's not possible it will display a Dialog which tells the User that
     * it's not possible.
     * <p>
     * 
     * @param code
     ...
     * Stop ki.
     * <p>
     * StopKI will try to interrupt the KI in its current calculations with
     * {@link mastermind.KI#stop()} The routine will wait till its clear that
     * the calculating Thread has stopped(
     * {@link mastermind.KI#isKICalculating()}).
     * <p>
     * After the calculating thread has stopped it will set a new State with
     * {@link mastermind.Mastermind#setState(State)}.
     * 
     ...
     * Adds the row.
     * <p>
     * This method will add a selected Code into the playfield(The code lies
     * within the parameter).
     * <p>
     * At first it will check if the user has already won or lose his game.
     * <p>
     * Next thing is it will calculate the difference between the added Row and
     * the Secret Code the difference will be stored in the locale variable red
     * and white.
     * <p>
     * Then it will add the complete row with the red and whites into the
     * playfield with the function {@link view.Mastermind_View#addRow(Row)}
     * <p>
     * It will also set at the end if you now won or lose the game with
     * {@link mastermind.Mastermind#setState(State)}.
     * <p>
     * 
     * @param code
    ...
    public void addRow(int[] code)
    {
        ...
        for (int i = 0; i < codeLength; i++)
        {
            ...
            {
                // Searches for if the color is within the Secret-Code(White
                // Ball)
                int j;
                for (j = 0; j < codeLength; j++)
                {
                    if (!mark[j] & (code[j] != secretCode[j])
                            & code[i] == secretCode[j])
                    {
                        break;
                    }
